...
export const CATEGORY_CONFIG: Record<TaskCategory, {
  ...
  Medications:       { bg: 'bg-blue-50',    text: 'text-blue-800',    border: 'border-blue-200',   headerBg: 'bg-blue-50',    circleBg: 'bg-blue-200',    Icon: PlusSolidIcon },
...
export function TaskBadge({ title, category }: { title: string; category: TaskCategory }) {
  ...
  const { Icon } = c;
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium border ${c.bg} ${c.text} ${c.border}`}>
...
export function VisitBadge({ title }: { title: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium border bg-sky-50 text-sky-800 border-sky-200">
...
export function OutcomeBadge({ title }: { title: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium border text-amber-800" style={{ backgroundColor: '#feefdc', borderColor: '#fcd9a8' }}>
